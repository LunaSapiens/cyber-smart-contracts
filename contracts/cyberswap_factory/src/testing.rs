...
fn create_pair() {
    ...
    deps = init(deps);
    deps.querier
        .with_cyberswap_factory(&[], &[("uusd".to_string(), 6u8)]);
    let assets = [
        Asset {
...
fn fail_to_create_pair_with_unknown_token() {
    ...
    deps.querier
        .with_cyberswap_factory(&[], &[("uluna".to_string(), 6u8)]);
...
fn reply_only_create_pair() {
    ...
    // register cyberswap pair querier
    deps.querier.with_cyberswap_factory(
        &[(
            &"0000".to_string(),
            &PairInfo {
